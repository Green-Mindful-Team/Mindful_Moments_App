...
import { getMoodColor, moodScalarClamped } from '../constants/moodScale';
...
export default function MoodTrackingScreen() {
  ...
  const statHeadingColor = isDark ? '#9CA3AF' : '#000000';
  ...
  const getChartData = () => {
    ...
    const data = moodHistory.map((item) => moodScalarClamped(item.mood));
  ...
  const getAverageMood = (): number => {
    ...
    const sum = moodHistory.reduce((acc, item) => acc + moodScalarClamped(item.mood), 0);
  ...
  const getMoodTrend = (): 'up' | 'down' | 'stable' => {
    ...
    // Series is oldest → newest. Compare the earlier half of the window vs the later half
    // so it works with ≤7 points (the old logic used slice(0, -7), which is empty until 8+ days).
    const mid = Math.floor(n / 2);
    const earlier = moodHistory.slice(0, mid);
    const later = moodHistory.slice(mid);
    if (earlier.length === 0 || later.length === 0) return 'stable';

    const avgMood = (items: typeof moodHistory) =>
      items.reduce((acc, item) => acc + moodScalarClamped(item.mood), 0) / items.length;

    const earlierAvg = avgMood(earlier);
    const laterAvg = avgMood(later);
    const diff = laterAvg - earlierAvg;

    // On a 1–3 scale, 0.2 ≈ one step of drift across the split; small enough to react, not noise-only
    const threshold = 0.2;
  ...
  const averageMoodDisplayTier =
    averageMood >= 2.5 ? 3 : averageMood >= 1.5 ? 2 : 1;
  ...
  const chartConfig = {
    ...
    // Dark mode: ash labels on x-axis (and y); light mode: dark gray
    labelColor: (opacity = 1) =>
      isDark ? `rgba(156, 163, 175, ${opacity})` : `rgba(17, 24, 39, ${opacity})`,
    style: {
      borderRadius: 16,
    },
    propsForDots: {
        ...
        <>
          <View style={styles.statsContainer}>
            <View style={[styles.statCard, { backgroundColor: colors.card}]}>
             <Text style={[styles.statLabel, { color: statHeadingColor }]}>Average Mood</Text>
              <View style={styles.statValueContainer}>
                <MoodGlyph
                  mood={averageMoodDisplayTier}
                  size={32}
                  color={averageMood >= 2.5 ? '#10b981' : averageMood >= 1.5 ? '#f59e0b' : '#ef4444'}
                />
                <Text style={[styles.statValue, { color: statHeadingColor }]}>{averageMood.toFixed(1)}</Text>
              </View>
            </View>

            <View style={[styles.statCard, {backgroundColor: colors.card}]}>
              <Text style={[styles.statLabel, { color: statHeadingColor }]}>Trend</Text>
              <View style={styles.statValueContainer}>
                <Ionicons
                ...
                />
                <Text style={[styles.statValue, { color: statHeadingColor }]}>
          ...
          <View style={[styles.legend, {backgroundColor: colors.card}]}>
            <View style={styles.legendItem}>
              <View style={[styles.legendColor, { backgroundColor: getMoodColor(1) }]} />
              <Text style={[styles.legendText, {color: colors.textMuted }]}>1 (Sad)</Text>
            </View>
            <View style={styles.legendItem}>
              <View style={[styles.legendColor, { backgroundColor: getMoodColor(2) }]} />
              <Text style={[styles.legendText, { color: colors.textMuted}]}>2 (Neutral)</Text>
            </View>
            <View style={styles.legendItem}>
              <View style={[styles.legendColor, { backgroundColor: getMoodColor(3) }]} />
              <Text style={[styles.legendText, { color: colors.textMuted }]}>3 (Very happy)</Text>
            </View>
          </View>
        </>
      )}
